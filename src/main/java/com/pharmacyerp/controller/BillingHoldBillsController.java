...
public class BillingHoldBillsController {
    ...
    private ObservableList<Sale> holdBills = FXCollections.observableArrayList();
            ...
            colAction.setCellFactory(param -> new TableCell<Sale, Void>() {
                private final Button resumeBtn = new Button("Resume");
                {
                    ...
                    resumeBtn.setOnAction(event -> {
                        ...
                        System.out.println("Resuming hold bill: " + sale.getInvoiceNo());
                        try {
                            ...
                            // Delete the hold bill so it isn't duplicated
    ...
    private void loadData() {
        holdBills.clear();
        holdBills.addAll(salesDAO.getSalesByStatus("HOLD"));
        if (tblHistory != null) {
            tblHistory.setItems(holdBills);
        }
        updateTotals();
    }
    
    private void updateTotals() {
        if (lblTotalItems != null) lblTotalItems.setText(String.valueOf(holdBills.size()));
        
        java.math.BigDecimal total = java.math.BigDecimal.ZERO;
        for (Sale s : holdBills) {
